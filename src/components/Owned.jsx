...
import {ERC1155, ERC1155ABI , ERC721, ERC721ABI, PROVIDER} from '../utils/contracts';
...
export default class Owned extends React.Component {
    ...
    fetch1 = async () => {
        const { address } = this.props;
        let contract = new ethers.Contract(ERC721, ERC721ABI, PROVIDER);
    ...
    fetch2 = async () => {
        const { address } = this.props;
        let contract = new ethers.Contract(ERC1155, ERC1155ABI, PROVIDER);
        let totalSupply = await contract.totalSupply()
        for(let i = 1; i <= totalSupply.toNumber(); i++){
            if(i == totalSupply.toNumber()){
